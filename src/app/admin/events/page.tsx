...
import { query, type Event } from "@/lib/db";
...
export default async function EventsPage() {
  ...
  const events = await query<Event>("SELECT * FROM events ORDER BY startDate DESC, createdAt DESC LIMIT 200");
    ...
    <main className="admin-shell min-h-dvh bg-[#f5f8f7] text-ink">
      <aside className="fixed inset-y-0 left-0 hidden w-72 border-r border-line bg-deep px-5 py-6 text-white lg:block">
        ...
        <nav className="space-y-1 text-sm">
          ...
          <Link href="/admin/events" className="block rounded-lg bg-white/12 px-3 py-2.5 font-semibold text-white">Event</Link>
      ...
      <section className="lg:pl-72">
        <header className="sticky top-0 z-20 border-b border-line bg-[#f5f8f7]/90 backdrop-blur">
          <div className="flex min-h-16 items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest text-dim">Event Management</p>
              <h2 className="font-display text-2xl font-semibold text-deep">สร้างและจัดการ Event</h2>
            </div>
            <LogoutButton />
          </div>
        </header>

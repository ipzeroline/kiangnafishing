...
import { query, type FishSpecies } from "@/lib/db";
import LogoutButton from "@/components/LogoutButton";
import FishSpeciesManager from "./FishSpeciesManager";
...
export default async function FishSpeciesPage() {
  ...
  const species = await query<SpeciesRow>(`
    SELECT fs.*,
      COALESCE(s.catchCount,0) catchCount,
      COALESCE(s.verifiedCount,0) verifiedCount,
      COALESCE(s.maxWeight,0) maxWeight
    FROM fish_species fs
    LEFT JOIN (
      SELECT species,
        COUNT(*) catchCount,
        COUNT(CASE WHEN status='VERIFIED' THEN id END) verifiedCount,
        COALESCE(MAX(CASE WHEN status='VERIFIED' THEN weightKg ELSE 0 END),0) maxWeight
      FROM catches
      GROUP BY species
    ) s ON s.species=fs.name
    ORDER BY fs.status ASC, fs.name ASC
  `);
    ...
    <main className="admin-shell min-h-dvh bg-[#f5f8f7] text-ink">
      <aside className="fixed inset-y-0 left-0 hidden w-72 border-r border-line bg-deep px-5 py-6 text-white lg:block">
        ...
        <nav className="space-y-1 text-sm">
          ...
          <Link href="/admin/fish-species" className="block rounded-lg bg-white/12 px-3 py-2.5 font-semibold text-white">ชนิดปลา</Link>
      ...
      <section className="lg:pl-72">
        <header className="sticky top-0 z-20 border-b border-line bg-[#f5f8f7]/90 backdrop-blur">
          <div className="flex min-h-16 items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest text-dim">Fish Catalog</p>
              <h2 className="font-display text-2xl font-semibold text-deep">จัดการชนิดปลา</h2>
            </div>
            <LogoutButton />
          </div>
        </header>
        <div className="px-4 py-6 sm:px-6 lg:px-8">
          <FishSpeciesManager species={species} />
        </div>
      </section>
    </main>
  );
